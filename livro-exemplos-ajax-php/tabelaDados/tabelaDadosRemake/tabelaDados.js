...
function EditarLinha(idLinha) {
	if(!linhaEmEdicao) {
		...
		celulas[2].innerHTML = '<input type="text" name="preco" id="formato-moeda" value="'+celulas[2].innerHTML+'">';
		celulas[3].innerHTML = '<a href="#" onclick="Atualizar()">Atualizar</a><br>'+
								'<a href="#" onclick="Cancelar()">Cancelar</a>';
		celulas[4].innerHTML = '&nbsp;';

		// OBJETOS (INPUT COM CONTEÚDO DINHEIRO), ESTAMOS INCLUINDO NELE ALGUMAS PROPRIEDADES
		verificaObjetoExiste('formato-moeda');
	}else 
...
function NovoRegistro() {
	...
	if(linhaEmEdicao)
		...
		novasCelulas[2].innerHTML = '<input type="text" name="preco" id="formato-moeda">'; // preço
...
function verificaObjetoExiste(idObjeto){
	if(document.getElementById(idObjeto) != null){
		...
		//
		objeto.addEventListener('keypress', function(event){
...
// FUNÇÃO USADA PARA FORMATAR CAMPOS MONETARIOS
